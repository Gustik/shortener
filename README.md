...
Профилирование проводилось с помощью `pprof` под нагрузкой через `wrk`.

### Что было оптимизировано

**1. `sync.Pool` для буфера чтения тела запроса (`ShortenURL`)**

До оптимизации хэндлер `ShortenURL` использовал `io.ReadAll`, который на каждый запрос выделял новый `[]byte`. Заменён на `sync.Pool` с `bytes.Buffer` — буфер переиспользуется между запросами.

Результат: аллокации в `ShortenURL` снизились на **~1536 КБ** (`-14.49%` от общего объёма).

**2. Замена `fmt.Sprintf` на конкатенацию строк**

Формирование полного короткого URL через `fmt.Sprintf("%s/%s", baseURL, shortURL)` заменено на `baseURL + "/" + shortURL`. Это убирает вызов `fmt.Sprintf`, который внутри использует `reflect` и аллоцирует промежуточные буферы.

Результат: снижение аллокаций при формировании URL в `ShortenURL`, `ShortenURLBatch`, `GetUserURLs`. (но наверное не очень заметно)

### Diff профилей (base vs result)
